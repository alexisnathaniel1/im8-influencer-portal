 ...
 *
 * Moves the full approve flow server-side so it is always audit-logged.
  ...
  try {
    ...
    if (sub.deliverable_id) {
      void admin
        ...
        .then(({ error }) => {
          if (error) console.warn("[approve] deliverable update failed", error);
        });
    }
    ...
    // Fire-and-forget Drive rename → _APPROVED
