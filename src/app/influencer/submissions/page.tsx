...
export default async function InfluencerSubmissionsPage() {
  ...
  const { data: submissions } = await supabase
    .from("submissions")
    .select("*, briefs(title), deals(influencer_name)")
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {!submissions?.length ? (
        <div className="bg-white rounded-xl border border-im8-stone/30 p-12 text-center text-im8-burgundy/40">
          No submissions yet.{" "}
          <Link href="/influencer/submit" className="text-im8-red hover:underline">Submit your first draft →</Link>
        </div>
      ) : (
            ...
            <div key={s.id} className="bg-white rounded-xl border border-im8-stone/30 p-5">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <div className="text-xs text-im8-burgundy/50 mt-1 flex items-center gap-2">
